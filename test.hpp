...
public:
    ...
    Node<Key, Value> * helper(Key k, Value v);
...
unsigned SkipList<Key, Value>::height(const Key & k) const {
    // TODO - your implementation goes here!
    unsigned counter = 0;
    Node<Key,Value> *copy = top_lvl->down; // The second layer from top
    while (copy != nullptr){
        if (!copy->next->isInfinity && copy->next->key <= k){
            copy = copy->next; // going right
        }
        else if (copy-> key == k){
            while(copy != nullptr){
                counter++;
    ...
    throw RuntimeException("The key doesn't exist in this Skip List.");
}
...
bool SkipList<Key, Value>::insert(const Key & k, const Value & v) {
    ...
    while(flipCoin(k,prevFlips) && maxLayers > current_lvl){
        ...
        if (current_lvl == layers){
            ...
            top_lvl->up = new Node<Key, Value>(true);
            top_end->up = new Node<Key, Value>(true);
